...
def map_constraint_stages(scenario, scenario_tree, LP_symbol_map):
    ...
    for block in reference_model.block_data_objects(active=True,
                                                    descend_into=True):
        ...
        for constraint_data in block.component_data_objects(Constraint,
            ...
            if piecewise_stage is None:
                constraint_node = scenario.constraintNode(constraint_data,
                                                          repn=canonical_repn,
...
def map_variable_stages(scenario, scenario_tree, LP_symbol_map):
    ...
    for scenario_tree_id, vardata in \
        ...
        try:
            ...
            foobar
        if LP_name == "RHS":
            raise RuntimeError("Congratulations! You have hit an edge case. The SMPS input "
...
def EXTERNAL_convert_explicit_setup(scenario_tree_manager,
    ...
    #
    with pyomo.repn.plugins.cpxlp.ProblemWriter_cpxlp() as writer:
        lp_filename = os.path.join(output_directory, basename+".lp."+scenario._name)
        assert 'column_order' not in io_options
        assert 'row_order' not in io_options
    ...
    # Write the .tim file
    #
    with open(os.path.join(output_directory, basename+".tim."+scenario._name),'w') as f:
        f.write('TIME\n')
        f.write('PERIODS\tLP\n')
        f.write('\t'+str(StageToVariableMap[firststage._name][0][0])+
                '\t'+str(LP_symbol_map.byObject[id(scenario._instance_objective)])+
                '\tTIME1\n')
    ...
    with open(os.path.join(output_directory,
        ...
        if stochastic_objective is not None:
            ...
            # zero coefficient from being eliminated from the
            # canonical representation
            objective_repn = generate_canonical_repn(scenario._instance_objective.expr,
                                                     compute_values=False)
            assert isinstance(objective_repn, LinearCanonicalRepn)
            for vardata in stochastic_objective:
...
def convert_explicit(output_directory,
    ...
    if isinstance(scenario_tree_manager._solver_manager,
        ...
        for scenario in scenario_tree._scenarios:
            ahs.append(scenariotreeserverutils.\
                       transmit_external_function_invocation_to_worker(
                           ...
                           function_args=(scenario_directory, basename, io_options),
    ...
    rc = os.system('cp '+os.path.join(scenario_directory,
                                      (basename+".lp."+
    ...
    for scenario in scenario_tree._scenarios:
        ...
        rc = os.system('cat '+scenario_sto_filename+" >> "+sto_filename)
        assert not rc
