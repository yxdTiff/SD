...
def transmit_scenario_tree_ids(scenario_tree_manager):
    ...
    generate_responses = scenario_tree_manager._options.handshake_with_phpyro
...
def release_scenariotreeservers(scenario_tree_manager):
    ...
    for job, worker in iteritems(scenario_tree_manager._phpyro_job_worker_map):
        scenario_tree_manager._solver_manager.queue(
            action="release",
            name=worker,
            object_name=job,
            generateResponse=False)
    scenario_tree_manager._phpyro_worker_jobs_map = {}
    scenario_tree_manager._phpyro_job_worker_map = {}
...
def initialize_scenariotree_workers(scenario_tree_manager):
    ...
    for worker_name in itertools.cycle(
        ...
        scenario_tree_manager._phpyro_worker_jobs_map.setdefault(worker_name,[]).append(job_name)
        scenario_tree_manager._phpyro_job_worker_map[job_name] = worker_name
